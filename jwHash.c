...
Latest source code you can find here: https://github.com/watmough/jwHash
...
*/
static unsigned long hashInt(unsigned long int x)
{
	x = ((x >> 16) ^ x) * 0x45d9f3b;
	x = ((x >> 16) ^ x) * 0x45d9f3b;
	x = ((x >> 16) ^ x);
...
void *delete_hash(jwHashTable *table)
{
    size_t i =0 ;
...
HASHRESULT add_str_by_int( jwHashTable *table, unsigned long int key, char *value )
{
    /* compute hash on key */
    unsigned long hash = hashInt(key) % table->buckets;
...
HASHRESULT add_dbl_by_int(jwHashTable* table, unsigned long key, double value )
{
    /* compute hash on key */
...
HASHRESULT add_int_by_int(jwHashTable* table, unsigned long key, long int value )
{
    /* compute hash on key */
	size_t hash = hashInt(key) % table->buckets;
...
/* Delete by int */
HASHRESULT del_by_int( jwHashTable* table, unsigned long key )
{
    /* compute hash on key */
	size_t hash = hashInt(key) % table->buckets;
...
HASHRESULT get_str_by_int( jwHashTable *table, unsigned long key, char **value )
{
    /* compute hash on key */
	size_t hash = hashInt(key) % table->buckets;
