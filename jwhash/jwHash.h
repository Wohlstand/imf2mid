...
/* Disabled, because compatibility with ANSI-C. Wohlstand
#ifdef HASHDEBUG
...
struct jwHashTable
{
    ...
    unsigned long bucketsinitial;          /* if we resize, may need to hash multiple times */
...
HASHRESULT add_str_by_int(jwHashTable *, unsigned long key, char *value);
HASHRESULT add_dbl_by_int(jwHashTable *, unsigned long key, double value);
HASHRESULT add_int_by_int(jwHashTable *, unsigned long key, long int value);
/* HASHRESULT add_ptr_by_int(jwHashTable *, long int key, void *value); */

/* Delete by int */
HASHRESULT del_by_int(jwHashTable *, unsigned long key);
